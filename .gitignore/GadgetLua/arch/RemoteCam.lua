...
function Gadget:DestroyObjects(t,selfCall)
    if      type(t) == "nil"      and  self.GameObjects and not selfCall             then  if self:DestroyObjects(self.GameObjects,true) then ret = true ; return true end
...
--          self:Teleport(self.aimedAtTeleportLocation)
            Camera.main.transform.position = self.aimedAtTeleportLocation[2].transform.position
        end
        self.showing = false
        self:DestroyObjects()
      end
