...
function Gadget:AimCheck() 
    ...
    then   
        ...
        if  Slua.IsNull(self.aimedAtNode) == false then
            ...
            self.aimedAtNode.transform.sizeDelta = Vector2(16,16)                          -- reset the size of the rect
            self.aimedAtNode:GetComponent("CanvasGroup").alpha = 0.2                       -- bring alpha back down
            if Slua.IsNull(self.aimedAtNode.transform:Find("Display")) == false  then      -- remove the panel with name on it
              GameObject.Destroy(self.aimedAtNode.transform:Find("Display").gameObject)
            end
        end
    

    elseif  cl.node and not Slua.IsNull(cl.node)
    then
