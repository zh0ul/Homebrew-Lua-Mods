...
function Gadget:GetRigidBody()

  if Slua.IsNull(self.player) then 
    local player = GameObject.Find("Player")
    if    Slua.IsNull(player)
    then  return
    else  self.player = player
    end
  end

  if Slua.IsNull(self.rigidbody) then 
    local rb = self.player.gameObject:GetComponent("Rigidbody")
    if    Slua.IsNull(rb)
    then  return
    else  self.rigidbody = rb
    end
  end
